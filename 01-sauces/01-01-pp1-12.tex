...
\hypertarget{traitementdeselementsdebase}{%
\section{ソースのベース作り}\label{traitementdeselementsdebase}}

\hypertarget{traitement-des-elements-de-base}{%
\subsection{Traitement des Éléments de Base dans le Travail des
Sauces}\label{traitement-des-elements-de-base}}
...
な物質が含まれていないために、全体の分量比率を考え直すことになる。
...
払えば、ルーに用いたバターを回収するのはさして難しいことではない\footnote{ソースに充分にとろみが付いている場合、乳化（エマルジョン化）
  している場合には回収そのものが困難であることは、日本の家庭料理のカ
  レーやシチューの例を見ればすぐにわかるだろう。また、この訳の底本と
  している第四版ではバターの使用を強調しているが、既に述べたように初
  版〜第三版まではグレスドマルミットまたはバターという指示であったこ
  とに留意する必要があるだろう。いえ、実際のところ、良質のバターを用
  いてルーを作ったほうが、軽やかな仕上りのソースになる傾向があること
  は言うまでもないだろう。}。それ を後で他の用途で使えばいいだろう。
