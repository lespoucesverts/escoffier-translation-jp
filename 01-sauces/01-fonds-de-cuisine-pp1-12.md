...
### 魚のフォン（フュメドポワソン[^n-1]）FONDS, OU FUMET DE POISSON

[^n-1]本質的には前出の「フォン」と同様のものだが、魚（およびジビエ）を素材としたフォンは香りがポイントとなるため、フュメ fumet (香気、良い香りの意)の名称のほうが一般的に使われている。

（仕上り10L分）

**主素材**……舌びらめ、メルラン[^13]やバルビュ[^14]のあら10kg。

**香味素材**……薄切りにした玉ねぎ500g、パセリの根[^15]と茎100g、マッシュルームの切りくず250g、レモンの搾り汁１個分、粒こしょう15g（これはフュメを漉す10分前に投入する）。
...
[^15]: パセリには根がにんじん形に肥大する品種もある（葉は平らでイタリアンパセリのように使う）。フランス料理で一般的に使われる葉の縮れるタイプと香りはやや異なる。
