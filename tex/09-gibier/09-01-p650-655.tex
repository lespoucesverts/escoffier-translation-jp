...
\hypersetup{
            ...
            pdfauthor={五 島　学（訳・注釈）; 河井 健司（訳・注釈）; 春野 裕征（訳）; 山本 学 （訳）},
            pdfborder={0 0 0},
            breaklinks=true}
...
\author{五 島　学（訳・注釈） \and 河井 健司（訳・注釈） \and 春野 裕征（訳） \and 山本 学 （訳）}
\date{}
%%%%%%%%%%%%%%%%%%%%%%% added by mgoto
\usepackage{setspace}
...
\hypertarget{ux731fux7363}{%
\section{猟獣}\label{ux731fux7363}}

赤鹿\footnote{cerf(セール)
  日本鹿、エゾ鹿もこれに含まれる。肉質が固いとしてあまりフランスでは好まれない。}、ダマ鹿、イザール\footnote{isard
  ピレネー山脈に生息するカモシカの一種。若いものが肉質が柔らかく特に珍重される。}、シャモワ\footnote{アルプス山脈などに生息するカモシカの一種で前出のイザールに非常に近い。}などはシュヴルイユと同様に、フランスで使われる大型ジビエに分類され、シュヴルイユと同様の調理をする。したがって、これらについては個別に項目を立てる必要はないだろう。

シュヴルイユのマリネはしてもしなくてもいいが、とりわけ若い、つまり肉質の柔らかい固体の場合にはマリネしない方がいい。

マリナードの作り方は、ソースの章で記してあり、肉がマリナードに浸かっている時間でマリネのされ方が決まり、そして主にシュヴルイユの場合によく見極める必要があるが、とりわけ肉が柔らかい場合にもマリネのされ方が決まる。

\hypertarget{chevereuil}{%
\subsection{シュヴルイユ}\label{chevereuil}}
...
\subsubsection{シュヴルイユのシヴェ}\label{civet-de-chevreuil}}
...
シュヴルイユの血液保存の難しさを考慮して、このとろみ付けには野ウサギの血、やむを得ない場合には養殖のウサギの血を用いる。
...
\subsection[コトレットとノワゼット]{\texorpdfstring{コトレットとノワゼット\footnote{牛の背肉、仔牛モモ肉中心部の名称「ノワ」よりも小さい、主に羊・仔羊・鹿のフィレ肉または背肉の中心部。}}{コトレットとノワゼット}}\label{cotelettes-et-noisettes}}

シュヴルイユのコトレットとノワゼットは同類で、仔羊の鞍下肉のようにたいてい2切れで1人前と数える。

他に調理法指定がなければ、油で手早くソテーしたあと余分な油をきる。バターで揚げたハート型または丸型の薄いクルトンと交互に王冠状に盛る。
...
\subsubsection{シュヴルイユのコトレット・ボーヴァル}\label{les-cotelettes-de-chevreuil-beauval}}
