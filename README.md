...
* 数字は一桁のものも半角数字を使い、前後を半角スペースで空けてください。
* 単位を表す ml, dl, L, g, kg, mm, cm なども前の数字との間に半角スペース1つを入れてください。
* 上記のような単位を示す半角文字（とりわけ L ）の後に役物（とくに句読点）が来る場合には、前後に半角スペースを入れてください。（例）1（半角スペース）L（半角スペース）。
