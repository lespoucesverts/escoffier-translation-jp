...
「チラ見」用のPDFは<>code の最上位ディレクトリにあります。ファイル名は le-guide-culinaire-exemple.pdf です。拡張子にご注意ください。最後が .tex となっているファイルはPDF組版用のものでTeXという組版言語を用いて書かれています。もしお読みになって目まい、頭痛になったとしても自己責任となりますのでご注意を。PDFファイルはブラウザ上でも直接開けるみたいですが、動作がとても遅い可能性があります。

また、「チラ見」ファイルの内容は、翻訳作業の進行に応じて、その時点で翻訳がひととおり完了した「節」となります。こまめにチェックすると、「チラ見」だけで一冊全部読みきることが出来るかもしれません。

また、『料理の手引き』の初版から第四版までの序文すべてと、初版の序文の後に付された「盛り付けをシンプルにする……」と「はしがき」を収録したPDF、escoffier-introductions.pdfもあります。フランス料理人およびエスコフィエに興味をお持ちの方はぜひご覧ください。

いずれのPDFも、ある程度ですがスマートフォンの画面でも多少は見やすいよう配慮して、判型はB5前提ですが、1ページあたりの行数を多くしています。このため、印刷には必ずしも適していません。
...
のように、直前の注釈番号にbisをつけてください。すでに直前のものにbisがついている場合は、bis2、bis3のようにして、かならず番号がダブらないようご注意ください。	
