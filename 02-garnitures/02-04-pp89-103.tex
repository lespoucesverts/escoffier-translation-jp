...
\tightlist
\item
  このガルニチュールは\protect\hyperlink{garniture-jardiniere}{ジャルディニエール}とまったくおなじ構成要素だが、すべての材料を混ぜてあえてしまう点が異なる。これを野菜料理用の深皿\footnote{timbale
    （タンバール）。円筒形でやや丈の低い型に詰めた仕立てもタンバルと呼び、また型も
    moule à timbale
    （ムーラタンバール）というい、かなりまぎらわしいので注意。}に別途盛り付けるか、アーティチョークの基底部に詰めるか、もしくは皿の中心にドーム状に盛り、その周囲に肉料理を並べるようにして盛り付ける。
\end{itemize}
...
\subsubsection{ガルニチュール・マレシェール{[}\^{}167{]}}\label{garniture-a-la-maraichere}}
...
\item
  50〜60gのトリュフをスライスし、イタリア風にあえる\footnote{詳細不明。原文
    lié à l'italienne
    英訳第5版では「\protect\hyperlink{sauce-italienne}{イタリア風ソース}であえる」としているが、初版〜第三版にいたるまで、1907年の英訳も含めてこれについての具体的な記述は見あたらない。}。\href{下記172の注で参照したのは英訳5版ですが、lames\%20de\%20truffe\%20liées\%20en\%20italienne\%20が「具体的に何を意味しているのか、特定できていません。}{}
\item
  マデイラ酒風味の\protect\hyperlink{sauce-demi-glace}{ソース・ドゥミグラス}
\end{itemize}
...
\item
  ...
  \(\frac{1}{3}\)量のトリュフのみじん切りを混ぜたイギリス式パン粉衣を付けて揚げ焼きしたもの\footnote{初版「これらの素材はパン粉衣を付けるか、トリュフのみじん切りをまぶし付けるか、パン粉の\(\frac{1}{3}\)量のトリュフを混ぜた衣を付けて調理する」。第二版〜第三版「これらの素材は必ず、トリュフのみじん切りをまぶし付けるか、細かい生パン粉に\(\frac{1}{3}\)量のトリュフのみじん切りを混ぜた衣を付けて調理する」。}。
\end{itemize}
\end{recette}
\href{初版〜3版書き方だと、バターを塗らないと付くわけがないですから、明らかにフランス式ですが、注記すべきでしょうか?\%20また、現行版でいきなりイギリス式になっているということは、加熱方法も変化があった可能性が考えられると思います（フライパンからフリトゥーズへ、みたいな）}{}
